		...
		do
			...
			if Result then
				...
				l_equal := true
					i := 1
				until
					i > v1.precise_out.count or l_equal = false  --must include l_equal = false or else l_equal will only really care about the last value checked
				loop											--ex) in 123.00 and 213.00, the loop l_equal will only really care about the last value, being the 2 0s.
...
feature -- tests
	...
	t2: BOOLEAN
		...
		do
			...
			check Result end
			Result := equal_values (v1,v4)  --fails when changed to v5, says check assertion violated
			check Result end
			Result := not equal_values (v1,v5)

			-- see what happens if you change v4 to v5
			assert_equal ("should be equal:", v1.precise_out, v4.precise_out)  -- what does this do?
		end
	...
	t3: BOOLEAN
		...
		do
			...
			b.deposit ("120.455")
			Result := a /= b and a ~ b --a /= b return true because they are 2 different objects, a ~ b returns true because a and b contain same fields
	...
	t4: BOOLEAN
		...
		do
			...
			assert ("Is Steve's account ok?",
				a.name ~ "Steve" and a.balance = zero, c)  --check assertion is violated, pretty much just another form of checking
